...
function Coursematerial(props) {
  ...
  const [courseidDataArry, setcourseidDataArry] = useState([]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      axios.get(url).then(
        (res) => {
          setcourseidDataArry(res.data.data);
          setisloading(false);
          console.log("courseIddata ", res);
        },
        (error) => {
  ...
  const getAllstudyMaterial = (ID) => {
    let id = ID;
    setcourse(getAllstudyMaterial);
    try {
      setisloading(true);

      let url = getBaseUrl() + `/api/studymaterial/getall/${id}`;
      axios.get(url).then(
        (res) => {
          setstudyDataArry(res.data.data);
          setisloading(false);
          console.log("getstudymaterialgetdata ", res);
        },
        (error) => {
          setisloading(false);
        }
                  ...
                  <div className=" mt-1 mr-2">
                    <div class="form-group mr-2">
                      <select
                        ...
                        value={course}
                        onChange={(e) => {
                          getAllstudyMaterial(e.target.value);
                        }}
                      >
                        <option>Select ....</option>
                        {courseidDataArry.map((row, index) => (
...
export default HOC(Coursematerial);
